...
class SignRecorder(object):
    def __init__(self, reference_signs: pd.DataFrame, seq_len=25):
    ...
    def process_vid(self,results) -> (str, bool):
        """
        Process a sequence of results to compute sign distances and predict the word.
    
        :param results_sequence: Sequence of mediapipe output results.
        :return: Tuple containing the predicted word and the recording state.
        """
        # # Extend the recorded results with the results_sequence
        # self.recorded_results.extend(results_sequence)
    
        # if len(self.recorded_results) >= self.seq_len:
        #     # If enough results are accumulated, compute distances
        #     self.compute_distances()
        #     print(self.reference_signs)
        ...
        # Return the predicted word and the recording state
        return self._get_sign_predicted(), self.is_recording
